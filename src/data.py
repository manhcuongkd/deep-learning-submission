...
def get_data_loaders(
    ...
    data_transforms = {
        "train": transforms.Compose([
            ...
            transforms.RandomAffine(scale=(0.9, 1.1), translate=(0.1, 0.1), degrees=10),  # Apply random affine transformations
            transforms.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5, hue=0.5),  # Apply random color modifications
            transforms.RandomHorizontalFlip(0.5),  # Apply horizontal flip with 50% probability
            transforms.RandomCrop(224, padding_mode="reflect", pad_if_needed=True),  # Random crop to 224x224
            transforms.RandAugment(num_ops=2, magnitude=9, interpolation=transforms.InterpolationMode.BILINEAR),  # Apply RandAugment
            transforms.ToTensor(),  # Convert to PyTorch tensor
            transforms.Normalize(mean, std)  # Normalize using computed mean and std
        ]),
